    ...
    public class PlayerDataManager : Singleton<PlayerDataManager>
    {
        ...
        private async void LoadPlayerDataAsync()
        {
            try
            {
                ...
                var savedData =
                    await CloudSaveService.Instance.Data.Player.LoadAsync(new HashSet<string> { "PlayerData" });
                Debug.Log($"PlayerData: {savedData["PlayerData"].Value.GetAsString()}");
                if (savedData["PlayerData"]?.Value != null) LoadJsonData(savedData["PlayerData"].Value.GetAsString());
            }
            catch (Exception e)
            {
                Load();
        ...
        public void Save()
        {
            ...
            var jsonString = JsonConvert.SerializeObject(playerData, settings);
